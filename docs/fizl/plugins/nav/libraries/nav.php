...
class Nav extends Plugin {
	...
	function auto()
	{
		...
		$map = directory_map(FCPATH.$this->CI->load->get_var('site_folder').'/'.$start);
		...
		if(in_array('order.txt', $map)):
			...
			$order = trim(read_file(FCPATH.$this->CI->load->get_var('site_folder').'/'.$this->start.'/order.txt'));
	...
	function order_items($arr)
	{
		...
		$loc = array_merge(array(FCPATH.$this->CI->load->get_var('site_folder').'/'.$this->start), $this->stack);
