...
class Asset {
	 ...
	 *             are stored.
	 */
	protected static $cache_path = 'resources/compiled/';
	...
	*/
	...
	public static function render_js($group = false, $inline_dep = null, $attr_dep = array())
	{
		...
		foreach ($file_groups as $group_name => $file_group)
		{
			...
			if (self::$groups['js'][$group_name]['combine'])
			{
				...
				if ($inline)
				{
					$ret .= self::html_tag('script', $attr, PHP_EOL.file_get_contents(basename( APPPATH ) .'/' .self::$cache_path.$filename).PHP_EOL).PHP_EOL;
				}
				else
				{
					$filepath = self::process_filepath(basename( APPPATH ) .'/' .self::$cache_path.$filename, 'js');
	...
	public static function render_css($group = false, $inline_dep = null, $attr_dep = array())
	{
		...
		foreach ($file_groups as $group_name => $file_group)
		{
			...
			if (self::$groups['css'][$group_name]['combine'])
			{
				...
				if ($inline)
				{
					$ret .= self::html_tag('style', $attr, PHP_EOL.file_get_contents( basename( APPPATH ) .'/' . self::$cache_path.$filename).PHP_EOL).PHP_EOL;
				}
				else
				{
					$filepath = self::process_filepath(basename( APPPATH ) . '/' . self::$cache_path.$filename, 'css');
	...
	protected static function find_files($file, $asset_type)
	{
		...
		{
			throw new Asset_Exception("Could not find namespace {$file}");
		}
		...
		{
			$glob_files = glob($file);

			if (!$glob_files || !count($glob_files))
			{
				throw new Asset_Exception("Found no files matching $file");
			}
	...
	public static function render_js_inline()
	{
		...
		foreach (self::$inline_assets['js'] as $content)
		{
			if ( !empty( $content ) ) {
				...
				$content_inline .= $content.PHP_EOL;
			}
		}

		if ( !empty( $content_inline ) ) {

			return self::html_tag('script', $attr, PHP_EOL.$content_inline).PHP_EOL;
		}
	...
	public static function render_css_inline()
	{
		...
		if ( !empty( $content_inline ) ) {
			...
			return self::html_tag('style', $attr, PHP_EOL.$content_inline).PHP_EOL;
		}
